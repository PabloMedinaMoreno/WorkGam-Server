 ...
 */
export const createWorkerService = async ({ username, email, password, gender, phone, role_id }) => {
  ...
  try {
    ...
    await pool.query("INSERT INTO gamification (employee_id) VALUES ($1)", [personId]);

    // Create a welcome notification
    await createAndSendNotificationService(personId, "¡Bienvenido a la plataforma!");
 ...
 */
export const updateWorkerService = async (workerId, { username, email, role_id, phone }) => {
  try {
    ...
    await pool.query("UPDATE employee SET role_id = $1 WHERE id = $2", [role_id, workerId]);

    // Get the updated worker's information with the role
    const role = await pool.query("SELECT name FROM role WHERE id = $1", [role_id]);
